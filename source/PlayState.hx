...
class PlayState extends MusicBeatState
{
	...
	public static var wasInMultiverseMenu:Bool = false;
	...
	public var flippedHealth:Float = 1;
	...
	public var doModcharts:Bool = false;
	...
	override public function create()
	{
		...
		ModchartLuaFuncs.luaFunctionStuff();
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...Note.ammo[mania])
		{
			...
			if (player < 1 && !ClientPrefs.opponentStrums) targetAlpha = 0;
	...
	function changeMania(newValue:Int)
	{
		...
		//fuck your shitty mania change
		//i still hate you for stealing my code
		/*
		if (!isStoryMode)
		{
		...
		{
			unspawnNotes[i].mania = mania;
			unspawnNotes[i].applyManiaChange();
		}
		*/

		//var kc = Std.parseInt(value1);
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if(daNote.copyY)
				{
					...
					{
						if (daNote.animation.curAnim.name.endsWith('tail')) { //you fucking dumbass
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					/*
					var newState:MusicBeatState; 
					if (wasInMultiverseMenu)
						newState = new MultiverseMenuState();
					else 
						newState = new StoryMenuState();
					*/
					MusicBeatState.switchState(wasInMultiverseMenu ? new MultiverseMenuState() : new StoryMenuState());
